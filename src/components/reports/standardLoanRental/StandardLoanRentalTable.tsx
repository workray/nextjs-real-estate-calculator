...
import StandardLoanRentalItem from './StandardLoanRentalItem'

const defaultColumns: ColumnDef<TStandardLoanRentalTableData>[] = [
  {
    header: 'No',
    accessorKey: 'no',
    isPinned: true,
  ...
  return (
    <div className="w-full relative overflow-hidden pb-4">
      <table className="table-auto w-full overflow-auto block">
        ...
        <tbody>
          {data.map((item: TStandardLoanRentalTableData, index: number) => (
            <StandardLoanRentalItem
              key={item._id}
              data={item}
              reportId={reportId}
              index={index + 1}
